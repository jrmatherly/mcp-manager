-- Custom migration for database functions
-- These functions provide performance monitoring and analytics capabilities
...
BEGIN
    ...
    SELECT
        ...
        COUNT(*) FILTER (WHERE health_status IN ('unknown', 'unknown')) as degraded_servers,
...
BEGIN
    ...
    WITH server_stats AS (
        SELECT
            ...
            ms.uptime,
            ms.request_count as total_requests
        FROM mcp_server ms
    )
    SELECT
        server_id,
        ...
        ROUND(uptime::numeric, 2) as uptime,
        total_requests::bigint,
        ROUND(
            (COALESCE(uptime::numeric, 0) * 0.5 +
             (1 - LEAST(COALESCE(avg_response_time::numeric, 1000) / 1000, 1)) * 0.3 +
...
BEGIN
    ...
    LEFT JOIN mcp_server ms ON ms.id = cb.server_id
    ORDER BY
        CASE cb.state::text
            WHEN 'open' THEN 1
...
BEGIN
    ...
    SELECT
        ...
        cp.avg_wait_time_ms::numeric,
        cp.last_used
    FROM connection_pools cp
    LEFT JOIN mcp_server ms ON ms.id = cp.server_id
